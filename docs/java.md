...
Interview Answer:
> I use `String` for values, `StringBuilder` for repeated single-threaded assembly, and `StringBuffer` only when shared synchronization is genuinely required.
...
Interview Answer:
> The equality and hashing methods form one contract. Immutable keys prevent lost lookups and duplicate logical entries.
...
Interview Answer:
> These references alter reachability for specialized cases. I prefer explicit ownership and bounded caches over using them as a general leak fix.
...
Interview Answer:
> Correct double-checked locking needs a `volatile` instance and a second check under the lock. I normally prefer DI for application singletons.
...
Interview Answer:
> I use encapsulation and abstraction to protect invariants, polymorphism to vary behavior, and inheritance only when the subtype relationship is sound.
...
Interview Answer:
> A class describes structure and behavior; an object is a concrete runtime instance of that class.
...
Interview Answer:
> Inheritance is useful for a stable is-a relationship and substitutability. I do not use it only as a reuse shortcut.
...
Interview Answer:
> Composition reduces hierarchy coupling and works well with dependency injection and testing.
...
Interview Answer:
> An interface expresses a capability or boundary without coupling clients to one implementation.
...
Interview Answer:
> A functional interface is a single-abstract-method contract used to pass behavior as a value.
...
Interview Answer:
> Java 8 made interfaces more evolvable with default methods and added interface static methods. I keep defaults small because conflicts and hidden behavior can reduce clarity.
...
Interview Answer:
> Interfaces support multiple inheritance of contracts, while Java avoids multiple inheritance of class state and implementation.
...
Interview Answer:
> I choose an interface for a replaceable boundary and an abstract class when related subclasses need shared state or construction.
...
Interview Answer:
> Encapsulation is not only private fields; it prevents callers from putting an object into an invalid state.
...
Interview Answer:
> Polymorphism lets callers depend on what an object can do rather than its concrete type, reducing coupling.
...
Interview Answer:
> Overloading adds compile-time entry points; overriding replaces inherited behavior while preserving a polymorphic contract.
...
Interview Answer:
> Construction must establish the base before subclass behavior runs, so constructors should call private or final initialization helpers.
...
Interview Answer:
> Abstraction is the external model; encapsulation is the boundary protecting the internal representation.
...
Interview Answer:
> Anonymous classes are one-off implementations. I use a named class when behavior has meaningful identity or reuse.
...
Interview Answer:
> A constructor establishes initial invariants; a method represents an operation on the object or class.
...
Interview Answer:
> Instantiation answers which object was created; initialization answers what valid state it started with.
...
Interview Answer:
> Java passes a copy of every argument. An object argument copy points to the same object, which explains visible mutations but failed reference swaps.
...
Interview Answer:
> I use `int` for non-null numeric values and `Integer` when nullability or an object-based API is required.
...
Interview Answer:
> I use `==` for primitives or intentional identity checks and `equals()` for object value equality.
...
Interview Answer:
> String immutability improves safety and reuse. I use interning only when profiling supports it and use `equals()` for content.
...
Interview Answer:
> `final` restricts reassignment or inheritance; it does not guarantee deep immutability. I use try-with-resources or explicit lifecycle methods instead of finalization.
...
Interview Answer:
> I use static members for stateless utilities or true class-wide constants. Application dependencies are usually clearer as injected, scoped objects.
...
Interview Answer:
> Fail-fast behavior exposes accidental modification, but it is not synchronization. Correctness needs a snapshot, lock, or concurrent collection.
...
Interview Answer:
> I mainly use `Optional` for return values where absence is expected, making the API contract explicit.
...
Interview Answer:
> The choice depends on ownership. I use shallow copies for immutable or intentionally shared children and deep copies when the new object must own independent mutable state.
...
Interview Answer:
> I default to the narrowest visibility that preserves the design because smaller visibility reduces coupling.
...
Interview Answer:
> Runtime overriding applies to inherited visible methods, not private implementation details.
...
Interview Answer:
> The class context has no implicit receiver; a static method can access instance state only through a supplied object.
...
Interview Answer:
> A `HashSet` is effectively a map of keys. Its expected `add` and `contains` cost is O(1), provided equality and hashing are stable.
...
Interview Answer:
> `ArrayList` gives fast indexed access, but resizing occasionally costs O(n). I presize collections on hot paths when the expected size is predictable.
...
Interview Answer:
> I use `ConcurrentHashMap` for independent concurrent entries, but I design business-level atomicity separately for compound operations.
...
Interview Answer:
> I use `Comparator` when a type needs multiple orderings or sorting logic should stay outside the model.
...
Interview Answer:
> The collection should express the invariant: sequence implies `List`; uniqueness implies `Set`.
...
Interview Answer:
> I use `ArrayList` by default and choose synchronization based on the actual shared workflow, not merely the collection class.
...
Interview Answer:
> I choose the map from the invariant: hashing for lookup, linked order for iteration, and a tree for sorted operations.
...
Interview Answer:
> I use `Iterator` or modern collection APIs; `Enumeration` mainly appears in older APIs such as `Hashtable`.
...
Interview Answer:
> `HashMap` combines hashing for fast candidate selection with equality for correctness. I use stable keys and a suitable initial capacity.
...
Interview Answer:
> Generics move type errors to compile time and make APIs self-documenting, while I remember that primitives need wrappers and runtime type checks are limited.
...
Interview Answer:
> GC eligibility is based on reachability, not intent. I find the unexpected strong reference from a root and remove or scope it correctly.
...
Interview Answer:
> Java separates compilation from execution through bytecode and a platform-specific JVM.
...
Interview Answer:
> `main` is static because it is the startup boundary and must be callable without an instance.
...
Interview Answer:
> GC reclaims unreachable heap objects, but resource lifetime and lifecycle ownership must be explicit.
...
Interview Answer:
> Stack memory is thread-local execution state; heap memory holds shared object data.
...
Interview Answer:
> Reflection enables frameworks and tooling, but I prefer compile-time generation when safety, performance, or shrinker compatibility matters.
...
Interview Answer:
> I choose serialization based on the boundary. For Android IPC I prefer `Parcelable` or a URI, and for persistence I prefer an explicit schema. `transient` is only a field-inclusion rule.
...
Interview Answer:
> `throw` is an action inside a method; `throws` is part of the method contract.
...
Interview Answer:
> `finally` is best-effort cleanup. I use try-with-resources for closeable resources and persist critical state before termination.
...
Interview Answer:
> Try-with-resources makes ownership explicit and prevents resource leaks more reliably than manual `finally` cleanup.
...
Interview Answer:
> Externalization can control the serialized representation, but I use it only when that control justifies its maintenance cost.
...
Interview Answer:
> `volatile` is for visibility; atomic types or locks are for read-modify-write operations and coordinated state changes.
...
Interview Answer:
> A pool separates task submission from thread management. I size it for the workload, make overload visible, and define shutdown and failure behavior.
...
Interview Answer:
> `synchronized` protects a critical section and provides visibility. I avoid nested locks where possible and consider higher-level concurrency tools.
...
Interview Answer:
> I prevent deadlocks by defining lock order, avoiding nested locks, and using thread dumps to confirm circular waiting.
...
Interview Answer:
> The Looper is the event loop, the Handler is the posting API, and HandlerThread packages a background thread with a Looper. New work often fits coroutines or executors better.
...
Interview Answer:
> RxJava types communicate cardinality and failure behavior. I use `Flowable` when a producer can outpace a consumer.
...
Interview Answer:
> Schedulers are execution policy, not a magic speed switch. I match them to the workload and trace the result.
...
Interview Answer:
> Subjects are deliberate hot bridges, not a default state-management solution. I choose replay semantics from the consumer contract.
...
Interview Answer:
> Backpressure needs a business policy: preserve every event, discard stale data, sample, or fail visibly.
...
Interview Answer:
> Cold streams model per-subscriber work; hot streams model shared ongoing state or events.
...
Interview Answer:
> An executor separates task submission from thread management. I size it deliberately and propagate failure and cancellation.
...
Interview Answer:
> I consider `ArrayMap` for small maps and `SparseArray` for integer keys, while keeping `HashMap` when scale or general keys matter. I verify with profiling.
...
Interview Answer:
> Android IPC copies a representation of data, not a live heap reference. I keep payloads small and use a URI or durable storage for large data.
...
Interview Answer:
> I avoid Java `Serializable` for large or performance-sensitive Android handoffs because it is reflection-heavy. I use `Parcelable` for small platform payloads and repository-backed state for larger data.
...
Interview Answer:
> I start with the simplest design and introduce a pattern only when it clarifies a recurring constraint or makes change safer.
...
Interview Answer:
> Factory centralizes one creation decision; abstract factory keeps a compatible product family together. I avoid the latter without a real family.
...
Interview Answer:
> Creational patterns protect construction invariants or hide selection when direct construction has become difficult to evolve.
...
Interview Answer:
> A singleton can suit a truly process-wide resource, but I prefer dependency injection because it exposes dependencies, improves testability, and makes scope explicit.
